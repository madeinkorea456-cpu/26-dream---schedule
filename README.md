...
- 시작: 이름 선택/등록
- 내 시간표: 월~일 08:00~24:00 15분 단위로 붓(가능/수업/알바)을 골라 칠하기
...
- Supabase 테이블 `public.dream_schedule_members`에 이름/부서/캠퍼스투어 여부와
  시간표 3종(`avail_slots` 가능 / `class_slots` 수업 / `job_slots` 알바)이
  저장됩니다. 한 슬롯은 세 배열 중 하나에만 속합니다.
- 세 배열 모두 `요일*100 + 슬롯` 형태의 정수 배열입니다. 요일은 0(월)~6(일),
  슬롯은 0~63으로 08:00~24:00을 15분 단위로 나눈 값입니다. 예: 화요일 09:15 →
  `105`.
- 히트맵/전원 가능 요약/이벤트 배정 도우미는 모두 `avail_slots`(가능) 기준으로만
  계산됩니다. 수업·알바는 "표로 보기"에서 참고용 색으로만 표시돼요.
